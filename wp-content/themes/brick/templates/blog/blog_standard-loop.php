...
if (brick_qode_options()->getOptionValue( 'blog_standard_type_show_categories' )){
	$blog_show_categories = brick_qode_options()->getOptionValue( 'blog_standard_type_show_categories' );
}

$blog_show_comments = "yes";
if (brick_qode_options()->getOptionValue( 'blog_standard_type_show_comments' )){
	$blog_show_comments = brick_qode_options()->getOptionValue( 'blog_standard_type_show_comments' );
}
$blog_show_author = "yes";
if (brick_qode_options()->getOptionValue( 'blog_standard_type_show_author' )){
	$blog_show_author = brick_qode_options()->getOptionValue( 'blog_standard_type_show_author' );
}
$blog_show_like = "yes";
if (brick_qode_options()->getOptionValue( 'blog_standard_type_show_like' )) {
	$blog_show_like = brick_qode_options()->getOptionValue( 'blog_standard_type_show_like' );
}
$blog_show_ql_icon_mark = "yes";
$blog_title_holder_icon_class = "";
if (brick_qode_options()->getOptionValue( 'blog_standard_type_show_ql_mark' )) {
	$blog_show_ql_icon_mark = brick_qode_options()->getOptionValue( 'blog_standard_type_show_ql_mark' );
}
...
$blog_show_date = "yes";
if (brick_qode_options()->getOptionValue( 'blog_standard_type_show_date' )) {
	$blog_show_date = brick_qode_options()->getOptionValue( 'blog_standard_type_show_date' );
}
$blog_show_read_more_button = "yes";
if(brick_qode_options()->getOptionValue( 'blog_standard_type_read_more_button' )){
	$blog_show_read_more_button =  brick_qode_options()->getOptionValue( 'blog_standard_type_read_more_button' );
}

$blog_show_social_share = "no";
$blog_social_share_type = "dropdown";
if(brick_qode_options()->getOptionValue( 'blog_standard_type_select_share_option' )){
	$blog_social_share_type = brick_qode_options()->getOptionValue( 'blog_standard_type_select_share_option' );
}
if (brick_qode_options()->getOptionValue( 'enable_social_share' ) =="yes"){
	if (brick_qode_options()->getOptionValue( 'post_types_names_post' ) =="post"){
		if (brick_qode_options()->getOptionValue( 'blog_standard_type_show_share' )&& $blog_social_share_type == "dropdown") {
			$blog_show_social_share = brick_qode_options()->getOptionValue( 'blog_standard_type_show_share' );
		}
	}
}

$blog_ql_background_image = "no";
if(brick_qode_options()->getOptionValue( 'blog_standard_type_ql_background_image' )){
	$blog_ql_background_image = brick_qode_options()->getOptionValue( 'blog_standard_type_ql_background_image' );
}
			...
			<div class="post_content_holder">
							...
							<div class="video-wrap"  >
								<video class="video" poster="<?php echo esc_url(get_post_meta(get_the_ID(), "video_format_image", true));  ?>" preload="auto">
									...
									<object width="320" height="240" type="application/x-shockwave-flash" data="<?php echo esc_url(get_template_directory_uri().'/js/flashmediaelement.swf'); ?>">
										...
										<img src="<?php echo esc_url(get_post_meta(get_the_ID(), "video_format_image", true));  ?>" width="1920" height="800" title="<?php esc_attr_e( 'No video playback capabilities', 'brick' ); ?>" alt="<?php esc_attr_e( 'Video thumb', 'brick' ); ?>" />
									</object>
								</video>
							</div>
						</div>
				...
				<div class="post_text">
					<div class="post_text_inner">
						...
						<?php } ?>
						<?php if ( ! brick_qode_wp_link_pages_exist() ) {
							brick_qode_excerpt();
							
							if ( ! post_password_required() ) {
								brick_qode_read_more_button('blog_standard_type_read_more_button');
								
								if(brick_qode_options()->getOptionValue( 'blog_standard_type_show_share' ) == "yes" && $blog_social_share_type == "list") {
				...
				<div class="post_text_columns">
					<div class="post_text  <?php if($blog_ql_background_image == "yes") {if ( has_post_thumbnail() ) { ?> link_image" style="background:url(<?php echo esc_url($background_image_src); ?>); <?php } } ?>">
						<div class="post_text_inner">
							...
							<?php } ?>
							<?php if(brick_qode_options()->getOptionValue( 'blog_standard_type_show_share' ) == "yes" && $blog_social_share_type == "list") {
								echo do_shortcode('[no_social_share_list]'); // XSS OK
							}; ?>
						</div>
					</div>
