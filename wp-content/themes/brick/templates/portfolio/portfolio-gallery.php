...
if (brick_qode_options()->getOptionValue( 'portfolio_gallery_image_hover_style' )) {
	$portfolio_gallery_hover_type = brick_qode_options()->getOptionValue( 'portfolio_gallery_image_hover_style' );
}

//gallery hover separator with text
if (brick_qode_options()->getOptionValue( 'portfolio_gallery_image_hover_separator' )) {
	$portfolio_gallery_image_hover_separator = brick_qode_options()->getOptionValue( 'portfolio_gallery_image_hover_separator' );
}
...
if ((brick_qode_options()->getOptionValue( 'portfolio_title_margin_bottom' ) != '')
    || (brick_qode_options()->getOptionValue( 'portfolio_title_color' ))){
	
	if (brick_qode_options()->getOptionValue( 'portfolio_title_margin_bottom' ) != '') {
		$portfolio_title_style .= 'margin-bottom:'.esc_attr(brick_qode_options()->getOptionValue( 'portfolio_title_margin_bottom' )).'px;';
	}
	
	if (brick_qode_options()->getOptionValue( 'portfolio_title_color' )) {
...
} elseif(brick_qode_options()->getOptionValue( 'portfolio_columns_number' ) !== '') {
	$columns_number = 'v'.brick_qode_options()->getOptionValue( 'portfolio_columns_number' );
}
	...
	if (is_array($portfolio_images) && count($portfolio_images)){
		foreach($portfolio_images as $portfolio_image){
			...
			<?php if($portfolio_image['portfolioimg'] != ""){ ?>
				...
				$single_image_id = brick_qode_get_attachment_id_from_url($portfolio_image['portfolioimg']);
				...
				switch ($portfolio_video_type){
					case "youtube": ?>
						<?php if($lightbox_video_single_project == "yes"){ ?>
							<a class="lightbox_single_portfolio <?php echo esc_attr($columns_number); ?>" title="<?php echo esc_attr( $portfolio_image['portfoliotitle'] ); ?>" href="https://www.youtube.com/watch?feature=player_embedded&v=<?php echo esc_attr( $portfolio_image['portfoliovideoid'] ); ?>" data-rel="prettyPhoto[single_pretty_photo]">
					...
					case "vimeo": ?>
						<?php if($lightbox_video_single_project == "yes"){ ?>
							...
							?>
							<a class="lightbox_single_portfolio <?php echo esc_attr($columns_number); ?>" title="<?php echo esc_attr( $portfolio_image['portfoliotitle'] ); ?>" href="https://vimeo.com/<?php echo esc_attr( $portfolio_image['portfoliovideoid'] ); ?>" data-rel="prettyPhoto[single_pretty_photo]">
								<i class="fa fa-play"></i>
	...
	<div class="column1">
		<div class="column_inner">
			<div class="portfolio_single_text_holder">
				<<?php echo esc_attr($portfolio_title_tag); ?> class="portfolio_single_text_title" <?php brick_qode_inline_style($portfolio_title_style); ?>><span><?php the_title(); ?></span></<?php echo esc_attr($portfolio_title_tag); ?>>
			<?php the_content(); ?>
		</div>
	</div>
</div>
