...
function brick_qode_add_custom_nav_fields( $menu_item ) {
...
add_filter( 'wp_setup_nav_menu_item', 'brick_qode_add_custom_nav_fields' );
 ...
 * @since       1.0
 */
function brick_qode_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
...
// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'brick_qode_update_custom_nav_fields', 10, 3 );
 ...
 * @since       1.0
 */
function brick_qode_edit_walker( $walker, $menu_id ) {
	return 'BrickQodeWalkerNavMenuEditCustom';
}
// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'brick_qode_edit_walker', 10, 2 );

include_once BRICK_INCLUDES_ROOT_DIR . '/nav_menu/edit_custom_walker.php';

/* Custom WP_NAV_MENU function for top navigation */

if (!class_exists('BrickQodeWalkerNavMenu1')) {
	class BrickQodeWalkerNavMenu1 extends Walker_Nav_Menu {
	...
	// add classes to ul sub-menus
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && (brick_qode_options()->getOptionValue( 'page_transitions' ) == "0")):
			...
			$wide_background = '';
			if(brick_qode_options()->getOptionValue( 'enable_wide_manu_background' ) == "yes" ){
			...
			if($item->hide == ""){
                ...
                if($depth == 0 && brick_qode_options()->getOptionValue( 'enable_dropdown_menu_item_icon' ) == "yes" ){
				...
				if($args->has_children && $angle_icon_first_level != "") {
                    $item_output .= '<i class="q_menu_arrow fa '.$angle_icon_first_level.'"></i>';
                }
...
if (!class_exists('BrickQodeWalkerNavMenu2')) {
	class BrickQodeWalkerNavMenu2 extends Walker_Nav_Menu {
...
if (!class_exists('BrickQodeWalkerNavMenu3')) {
    class BrickQodeWalkerNavMenu3 extends Walker_Nav_Menu {
...
if (!class_exists('BrickQodeWalkerNavMenu4')) {
    class BrickQodeWalkerNavMenu4 extends Walker_Nav_Menu {
