...
$args = array_merge($args, brick_qode_icon_collections()->getShortcodeParams('content_menu_'));
...
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'vc_row wpb_row section '. ( $this->settings('base')==='vc_row_inner' ? 'vc_inner ' : '' ) . $el_class . vc_shortcode_custom_css_class( $css, ' ' ), $this->settings['base'], $atts );
...
$animation_style = "";
if($transition_delay != ""){
	$animation_style .= " transition-delay:" . $transition_delay . "ms; -webkit-animation-delay:" . $transition_delay . "ms; animation-delay:" . $transition_delay . "ms;";
}
if($animation_duration != ""){
	$animation_style .= " transition-duration:" . $animation_duration . "ms; -webkit-animation-duration:" . $animation_duration . "ms; animation-duration:" . $animation_duration . "ms;";
}
$anchor_id = "";
if($anchor != ""){
...
if($content_menu_title != ""){
	$menu_title = ' data-q_title="'.esc_attr($content_menu_title).'"';
}

$menu_icon = '';
if ($icon_pack !== '' && $in_content_menu !== '') {

	$item_collection_icon_obj = brick_qode_icon_collections()->getIconCollection($icon_pack);
...
if($row_type == 'row') {
	...
	if($video == "show_video"){
		...
		if($use_row_as_full_screen_section == "yes"){
			...
			if(!empty($video_webm)) { $output .= '<source type="video/webm" src="'.esc_url($video_webm).'">'; }
			if(!empty($video_mp4)) { $output .= '<source type="video/mp4" src="'.esc_url($video_mp4).'">'; }
			if(!empty($video_ogv)) { $output .= '<source type="video/ogg" src="'. esc_url($video_ogv).'">'; }
			$output .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.get_template_directory_uri().'/js/flashmediaelement.swf">
										...
										<img src="'.esc_url($v_image).'" width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'brick' ) . '"  alt="' . esc_attr__( 'Video thumb', 'brick' ) . '"  />
								</object>
						</video>';
		} else {
			$output .= '<div class="mobile-video-image" style="background-image: url('.esc_url($v_image).')"></div><div class="video-overlay';
			if($video_overlay == "show_video_overlay"){
				$output .= ' active';
			}
			$output .= '"';
			...
			$output .= '></div><div class="video-wrap">
								...
								<video class="video" width="1920" height="800" poster="'.esc_url($v_image).'" controls="controls" preload="auto" loop autoplay muted>';
			if(!empty($video_webm)) { $output .= '<source type="video/webm" src="'.esc_url($video_webm).'">'; }
			if(!empty($video_mp4)) { $output .= '<source type="video/mp4" src="'.esc_url($video_mp4).'">'; }
			if(!empty($video_ogv)) { $output .= '<source type="video/ogg" src="'. esc_url($video_ogv).'">'; }
			$output .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.get_template_directory_uri().'/js/flashmediaelement.swf">
	...
	if($row_type == "row" && $css_animation != ""){
		$output .= '<div class="'. $clsss_css_animation .'"><div '. brick_qode_get_inline_style($animation_style).' >';
	}
	...
	if($logo_in_content_menu == 'logo_in_content_menu'){
		...
		if( brick_qode_options()->getOptionValue( 'content_menu_logo_image' ) != ""){
			$logo_image = brick_qode_options()->getOptionValue( 'content_menu_logo_image' );
		}else if( brick_qode_options()->getOptionValue( 'logo_image' ) != ""){
			$logo_image = brick_qode_options()->getOptionValue( 'logo_image' );
		}
		$output .= '<a href="'.esc_url(home_url('/')).'"><img src="'.esc_url($logo_image).'" alt="' . esc_attr__( 'Logo', 'brick' ) . '" /></a>';
		$output .= '</div>';
	}
	...
	if($custom_widget_area !== ''){
		...
		$output .= brick_qode_get_dynamic_sidebar_content($custom_widget_area);
		$output .= '</div></div></div>';
	}
