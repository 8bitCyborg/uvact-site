...
class Carousel implements Lib\ShortcodeInterface {
     ...
     *
     * @see brick_qode_core_get_carousel_slider_array_vc()
     */
            ...
            vc_map( array(
                ...
                "params" => array(
                    array(
                        ...
                        "param_name" => "carousel",
                        "value" => brick_qode_core_get_carousel_slider_array_vc(),
                        "description" => ""
                    ),
                    array(
                        "type" => "dropdown",
    ...
    public function render($atts, $content = null) {
        ...
        if ($carousel != "") {
            ...
            $q = array('post_status' => 'publish', 'post_type' => 'carousels', 'carousels_category' => $carousel, 'orderby' => $orderby, 'order' => $order, 'posts_per_page' => '-1');
            ...
            if ($query_loop->have_posts()) : $post_count = 1;
                ...
                while ($query_loop->have_posts()) : $query_loop->the_post();
                    ...
                    if ($image != "") {
                        $html .= "<span class='first_image_holder " . $has_hover_image . "'><img src='" . esc_url( $image ) . "' alt='" . $title . "'></span>";
                    }

                    if ($hover_image != "") {
                        $html .= "<span class='second_image_holder " . $has_hover_image . "'><img src='" . esc_url( $hover_image ) . "' alt='" . $title . "'></span>";
                    }
