...
class BrickQodeImport {
	...
	public function import_content( $file ) {
		...
		require_once BRICK_CORE_MODULES_PATH . '/import/class.wordpress-importer.php';
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'qode_options_brick' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );
			
			update_option( 'qode_options_brick', $new_global_values );
		}
	}
	...
	function qode_admin_import() {
		...
		$this->pagehook = add_submenu_page(
			...
			array( $brick_qode_framework->getSkin(), 'renderImport' )
		);
		
		add_action( 'admin_print_scripts-' . $this->pagehook, 'brick_qode_enqueue_admin_scripts' );
		add_action( 'admin_print_styles-' . $this->pagehook, 'brick_qode_enqueue_admin_styles' );
	}
}

if ( ! function_exists( 'brick_core_initialize_import_object' ) ) {
	function brick_core_initialize_import_object() {
		$qode_import_object = brick_core_get_import_object();
	}
	
	add_action( 'init', 'brick_core_initialize_import_object' );
}

if ( ! function_exists( 'brick_core_get_import_object' ) ) {
	function brick_core_get_import_object() {
		$import_object = new BrickQodeImport();
		
		return $import_object;
	}
}

if ( ! function_exists( 'brick_core_data_import' ) ) {
	function brick_core_data_import() {
		$brick_core_import = brick_core_get_import_object();
		
		if ( $_POST['import_attachments'] == 1 ) {
			$brick_core_import->attachments = true;
		} else {
			$brick_core_import->attachments = false;
		}
		...
		$brick_core_import->import_content( $folder . $_POST['xml'] );
		$brick_core_import->update_meta_fields_after_import( $folder );
		
		die();
	}
	
	add_action( 'wp_ajax_brick_core_action_data_import', 'brick_core_data_import' );
}

if ( ! function_exists( 'brick_core_widgets_import' ) ) {
	function brick_core_widgets_import() {
		$brick_core_import = brick_core_get_import_object();
		...
		$brick_core_import->import_widgets( $folder . 'widgets.txt', $folder . 'custom_sidebars.txt' );
		
		die();
	}
	
	add_action( 'wp_ajax_brick_core_action_widgets_import', 'brick_core_widgets_import' );
}

if ( ! function_exists( 'brick_core_options_import' ) ) {
	function brick_core_options_import() {
		$brick_core_import = brick_core_get_import_object();
		...
		$brick_core_import->import_options( $folder . 'options.txt' );
		$brick_core_import->update_options_after_import( $folder );
		
		die();
	}
	
	add_action( 'wp_ajax_brick_core_action_options_import', 'brick_core_options_import' );
}

if ( ! function_exists( 'brick_core_other_import' ) ) {
	function brick_core_other_import() {
		$brick_core_import = brick_core_get_import_object();
		...
		$brick_core_import->import_options( $folder . 'options.txt' );
		$brick_core_import->import_widgets( $folder . 'widgets.txt', $folder . 'custom_sidebars.txt' );
		$brick_core_import->import_menus( $folder . 'menus.txt' );
		$brick_core_import->import_settings_pages( $folder . 'settingpages.txt' );
	...
	add_action( 'wp_ajax_brick_core_action_other_import', 'brick_core_other_import' );
}