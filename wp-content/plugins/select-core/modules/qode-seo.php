...
if ( ! function_exists( 'brick_qode_seo_plugin_installed' ) ) {
	/**
	 * Function that checks if popular seo plugins are installed
	 * @return bool
	 */
	function brick_qode_seo_plugin_installed() {
		return defined( 'WPSEO_VERSION' );
	}
}

if ( ! function_exists( 'brick_qode_remove_yoast_json_on_ajax' ) ) {
	/**
	 ...
	 * @version 0.2
	 */
	function brick_qode_remove_yoast_json_on_ajax( $data ) {
	...
	if ( brick_qode_seo_plugin_installed() && version_compare( WPSEO_VERSION, '1.6' ) >= 0 ) {
		add_filter( 'wpseo_json_ld_output', 'brick_qode_remove_yoast_json_on_ajax' );
		add_filter( 'disable_wpseo_json_ld_search', 'brick_qode_remove_yoast_json_on_ajax' );
	}
}

if ( ! function_exists( 'brick_qode_localize_no_ajax_pages' ) ) {
	/**
	 ...
	 *
	 * @uses brick_qode_get_objects_without_ajax()
	 * @uses brick_qode_get_wpml_pages_for_current_page()
	 * @uses brick_qode_get_woocommerce_pages()
	 *
	 * @version 0.1
	 */
	function brick_qode_localize_no_ajax_pages() {
		...
		if ( brick_qode_is_ajax_enabled() ) {
			...
			$no_ajax_pages = array_merge( $no_ajax_pages, brick_qode_get_objects_without_ajax() );
			
			//is wpml installed?
			if ( brick_qode_is_wpml_installed() ) {
				//get translation pages for current page and merge with main array
				$no_ajax_pages = array_merge( $no_ajax_pages, brick_qode_get_wpml_pages_for_current_page() );
			}
			
			//is woocommerce installed?
			if ( brick_qode_is_woocommerce_installed() ) {
				//get all woocommerce pages and products and merge with main array
				$no_ajax_pages = array_merge( $no_ajax_pages, brick_qode_get_woocommerce_pages() );
			}
			...
			wp_localize_script( 'brick-default-dynamic', 'no_ajax_obj', array(
				'no_ajax_pages' => $no_ajax_pages
			) );
		}
	}
	
	add_action( 'wp_enqueue_scripts', 'brick_qode_localize_no_ajax_pages' );
}

if ( ! function_exists( 'brick_qode_get_woocommerce_pages' ) ) {
	/**
	 ...
	 * @version 0.1
	 */
	function brick_qode_get_woocommerce_pages() {
...
if ( ! function_exists( 'brick_qode_get_objects_without_ajax' ) ) {
	/**
	 ...
	 * @version 0.1
	 */
	function brick_qode_get_objects_without_ajax() {
...
if ( ! function_exists( 'brick_qode_get_wpml_pages_for_current_page' ) ) {
	/**
	 ...
	 * @version 0.1
	 */
	function brick_qode_get_wpml_pages_for_current_page() {
...
if ( ! function_exists( 'brick_qode_add_ajax_meta' ) ) {
	/**
	 * Function that echoes meta data for ajax
	 */
	function brick_qode_add_ajax_meta() {
		$global_options  = brick_qode_return_global_options();
		
		if ( isset( $global_options['disable_qode_seo'] ) && $global_options['disable_qode_seo'] == 'no' && ! brick_qode_seo_plugin_installed() ) {
			$seo_description = get_post_meta( brick_qode_get_page_id(), "seo_description", true );
			$seo_keywords    = get_post_meta( brick_qode_get_page_id(), "seo_keywords", true );
			?>
	...
	add_action( 'brick_qode_action_ajax_meta', 'brick_qode_add_ajax_meta' );
}

if ( ! function_exists( 'brick_qode_add_header_meta' ) ) {
	/**
	 * Function that echoes meta data if our seo is enabled
	 */
	function brick_qode_add_header_meta() {
		global $is_IE;
		$global_options  = brick_qode_return_global_options();
	...
	add_action( 'brick_qode_action_header_meta', 'brick_qode_add_header_meta' );
}